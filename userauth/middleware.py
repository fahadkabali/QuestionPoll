    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            if user.user_type == '1':  # Admin
                if modulename == 'valuation_questions.views':
                    error = True
                    if request.path == reverse('detail'):
                        pass
                    else:
                        messages.error(
            ...
            if request.path == reverse('login') or request.path == reverse('signup') or modulename == 'django.contrib.auth.views' or request.path == reverse('login'):
                pass
            elif modulename == 'administrator.views' or modulename == 'valuation_questions.views':
